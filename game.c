...
game_play_player_card_selected (GameField* game_field, CardsHand* player_hand)
{
	...
	GtkCard* field_card = game_field_get_selected (game_field);

	if (player_card != NULL && field_card != NULL){
		gtk_card_unselect (player_card);
	    gtk_card_switch_content (player_card, field_card);
	    ...
	    gtk_widget_set_sensitive (GTK_WIDGET(gtk_card_get_button (field_card)), FALSE);
	    gtk_widget_set_name (GTK_WIDGET(gtk_card_get_button (field_card)), "togglebuttonuser");
...
game_play_cpu_card_random (GameField* game_field, CardsHand* cpu_hand)
{
	...
	GtkCard* field_card = game_field_get_random (game_field);

	if (cpu_card != NULL && field_card != NULL){
		gtk_card_write_label (cpu_card);
		gtk_card_switch_content (cpu_card, field_card);
	    gtk_widget_set_name (GTK_WIDGET(gtk_card_get_button (cpu_card)), "");
	    gtk_widget_set_sensitive (GTK_WIDGET(gtk_card_get_button (field_card)), FALSE);
	    gtk_widget_set_name (GTK_WIDGET(gtk_card_get_button (field_card)), "togglebuttoncpuplayed");
